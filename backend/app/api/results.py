...
async def create_result(result_data: ResultCreate, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        try:
            ...
            difficulty_multiplier = {"easy": 1, "medium": 1.5, "hard": 2}.get(result_data.difficulty, 1)
            performance_multiplier = percentage / 100  # 0-1 based on percentage
            xp_earned = int(base_xp * difficulty_multiplier * performance_multiplier)
            
            # Update user stats
            update_data = {
                "$inc": {
                    "xp": xp_earned,
                    "total_questions_answered": result_data.total_questions,
                    "correct_answers": correct_answers
                },
                "$set": {
            ...
            while current_xp >= ((new_level * 100) + ((new_level - 1) * 50)):
                new_level += 1
            ...
            print(f"[GAMIFICATION] [CREATE_RESULT] User {user_id} earned {xp_earned} XP, level: {new_level}")
            
        except Exception as gamification_error:
            print(f"[WARNING] [CREATE_RESULT] Gamification update failed for user {user_id}: {str(gamification_error)}")
            # Don't fail the result creation if gamification fails
