  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requestData = {
        ...
        hidden_test_cases: filteredTestCases.filter(t => t.is_hidden).map(testCase => ({
          input: testCase.input.trim(),
          expected_output: testCase.expected_output.trim()
        })),
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-2xl font-bold text-purple-200">Add Coding Question</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-purple-800 border border-purple-500 rounded-lg text-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500 h-32 resize-none"
            />
          </div>
            ...
            {formData.constraints.map((constraint, index) => (
              <div key={index} className="flex items-center space-x-2 mb-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-purple-800 border border-purple-500 rounded-lg text-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
                <button
          ...
          <div>
            ...
            {formData.examples.map((example, index) => (
              <div key={index} className="border border-purple-500/30 rounded-lg p-3 mb-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-2">
                  <div>
                    <label className="block text-purple-300 text-sm mb-1">Input</label>
                    <textarea
                      value={example.input}
                      onChange={(e) => handleArrayChange('examples', index, { ...example, input: e.target.value })}
                      placeholder="Input example..."
                      className="w-full px-2 py-1 bg-purple-800 border border-purple-500 rounded text-purple-200 text-sm resize-none h-16"
                    />
                  </div>
                  <div>
                    <label className="block text-purple-300 text-sm mb-1">Output</label>
                    <textarea
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1 mr-2">
                    <label className="block text-purple-300 text-sm mb-1">Explanation</label>
                    <input
                      type="text"
                      value={example.explanation}
                      onChange={(e) => handleArrayChange('examples', index, { ...example, explanation: e.target.value })}
                      placeholder="Explanation (optional)..."
                      className="w-full px-2 py-1 bg-purple-800 border border-purple-500 rounded text-purple-200 text-sm"
                    />
                  </div>
                  <button
            ...
            {formData.test_cases.map((testCase, index) => (
              <div key={index} className="border border-purple-500/30 rounded-lg p-3 mb-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-2">
                  ...
                  <div>
                    <label className="block text-purple-300 text-sm mb-1">Expected Output</label>
                    <textarea
                      value={testCase.expected_output}
                      onChange={(e) => handleArrayChange('test_cases', index, { ...testCase, expected_output: e.target.value })}
                      placeholder="Expected output..."
                      className="w-full px-2 py-1 bg-purple-800 border border-purple-500 rounded text-purple-200 text-sm resize-none h-16"
                    />
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => handleArrayChange('test_cases', index, { ...testCase, is_hidden: e.target.checked })}
                      className="rounded"
                    />
                    <span className="text-purple-300 text-sm">Hidden test case</span>
                  </label>
                  <button
        ...
        <div className="flex justify-end space-x-3 mt-6">
          <button
            onClick={onClose}
            className="px-6 py-2 bg-purple-800 hover:bg-purple-700 text-purple-200 rounded-lg transition-colors"
          >
            Cancel
          </button>
          <button
