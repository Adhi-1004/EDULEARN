                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className={`text-sm font-medium ${getActivityColor(user.activity_score)}`}>
                          {user.activity_score.toFixed(1)}
                        </span>
                      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900 dark:text-white">
                        {user.assessments_taken} assessments
                      </div>
                      <div className="text-sm text-gray-500 dark:text-gray-400">
                        Avg: {user.average_score.toFixed(1)}%
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div><span className="font-medium">Name:</span> {selectedUser.user.name}</div>
                  <div><span className="font-medium">Email:</span> {selectedUser.user.email}</div>
                  <div><span className="font-medium">Role:</span> {selectedUser.user.role}</div>
                  <div><span className="font-medium">Created:</span> {new Date(selectedUser.user.created_at).toLocaleDateString()}</div>
              ...
              {selectedUser.analytics && (
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div><span className="font-medium">Activity Score:</span> {selectedUser.analytics.activity_score.toFixed(1)}</div>
                    <div><span className="font-medium">Progress:</span> {selectedUser.analytics.progress_percentage.toFixed(1)}%</div>
                    <div><span className="font-medium">Assessments:</span> {selectedUser.analytics.assessments_taken}</div>
                    <div><span className="font-medium">Average Score:</span> {selectedUser.analytics.average_score.toFixed(1)}%</div>
                    <div><span className="font-medium">Badges:</span> {selectedUser.analytics.badges_earned}</div>
                    <div><span className="font-medium">Streak:</span> {selectedUser.analytics.streak_days} days</div>
                  </div>
                </div>
              )}
            </div>
