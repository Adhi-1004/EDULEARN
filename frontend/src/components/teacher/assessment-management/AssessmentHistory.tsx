...
const AssessmentHistory: React.FC<AssessmentHistoryProps> = ({
  ...
  const handleViewUpcoming = () => {
    window.location.assign('/teacher/upcoming-assessments')
  }
  ...
  return (
    <motion.div variants={ANIMATION_VARIANTS.slideUp} className="mb-8">
      <div className="grid grid-cols-1 gap-4">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold text-blue-200">Upcoming Tests</h2>
            <Button 
              variant="secondary" 
              size="sm" 
          ...
          {upcomingAssessments.length === 0 ? (
            <div className="text-center py-8">
              <div className="text-blue-300 mb-2">No upcoming tests</div>
              <p className="text-blue-400 text-sm">Schedule assessments to see them here</p>
            </div>
          ) : (
            <div className="space-y-3">
              {upcomingAssessments.slice(0, 5).map((assessment) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    {assessment.created_at && (
                      <div className="text-blue-400 text-xs">
                        Scheduled {new Date(assessment.created_at).toLocaleDateString()}
                      </div>
                    )}
                  </div>
                  <Button 
