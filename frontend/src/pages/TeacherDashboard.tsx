...
const TeacherDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const mockStudents: Student[] = [
        { id: "1", name: "Alice Johnson", email: "alice@example.com", progress: 85, lastActive: "2023-05-15", batch: "Batch A" },
        { id: "2", name: "Bob Smith", email: "bob@example.com", progress: 72, lastActive: "2023-05-14", batch: "Batch A" },
        { id: "3", name: "Carol Davis", email: "carol@example.com", progress: 91, lastActive: "2023-05-16", batch: "Batch B" },
        { id: "4", name: "David Wilson", email: "david@example.com", progress: 68, lastActive: "2023-05-12", batch: "Batch B" },
        { id: "5", name: "Eva Brown", email: "eva@example.com", progress: 79, lastActive: "2023-05-15", batch: "Batch C" },
      ];
      
      const mockBatches: Batch[] = [
        { id: "all", name: "All Students", studentCount: mockStudents.length, createdAt: "2023-01-01" },
        { id: "batch-a", name: "Batch A", studentCount: 2, createdAt: "2023-03-01" },
        { id: "batch-b", name: "Batch B", studentCount: 2, createdAt: "2023-03-15" },
        { id: "batch-c", name: "Batch C", studentCount: 1, createdAt: "2023-04-01" },
      ];
  ...
  const handleCreateBatch = async () => {
    ...
    try {
      ...
      showError("Error", "Failed to create batch");
    }
  };

  const handleAddStudentToBatch = async (batchId: string) => {
    if (!studentEmail.trim() && !studentName.trim()) {
      showError("Error", "Please enter either student email or name");
      return;
    }
    ...
    try {
      ...
      const response = await api.post(`/teacher/batches/${batchId}/students/add`, {
      ...
      const errorMessage = err.response?.data?.detail || "Failed to add student to batch";
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {filteredStudents.map((student, index) => (
                        ...
                        >
                          <td className="py-3 px-4 text-purple-200">{student.name}</td>
                          <td className="py-3 px-4 text-purple-300">{student.email}</td>
                          <td className="py-3 px-4">
                            <div className="flex items-center">
                              ...
                              <span className="text-purple-300 text-sm">{student.progress}%</span>
                            </div>
                          </td>
