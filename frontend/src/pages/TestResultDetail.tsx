...
import type { User, DetailedTestResult, QuestionReview } from "../types"
...
const TestResultDetail: React.FC<TestResultDetailProps> = ({ }) => {
  ...
  if (loading) {
    return (
      <PageShell title="Loading test result..." subtitle="Please wait">
  ...
  if (error) {
    return (
      <PageShell title="Error Loading Result" subtitle="Please try again">
        <div className="min-h-[60vh] flex items-center justify-center">
          <ErrorState
  ...
  if (!result) {
    return (
      <PageShell title="Result Not Found" subtitle="The requested test result could not be found">
        <div className="min-h-[60vh] flex items-center justify-center">
          <ErrorState
  ...
  return (
    <PageShell title="Test Result Details" subtitle={`${result.topic} • ${result.difficulty}`}>
      <motion.div
        initial="initial"
        animate="animate"
        exit="exit"
      ...
      >
        ...
        {/* Score Card */}
        <motion.div variants={ANIMATION_VARIANTS.slideUp} className="max-w-4xl mx-auto mb-8">
          <Card className="p-8 text-center">
            ...
            >
              <div
                className={`w-32 h-32 rounded-full bg-gradient-to-r ${getScoreColor(result.percentage)} flex items-center justify-center mx-auto mb-4 shadow-2xl`}
              >
                <span className="text-4xl font-bold text-white">{Math.round(result.percentage)}%</span>
              </div>
              <h3 className="text-2xl font-bold text-purple-200 mb-2">{getScoreMessage(result.percentage)}</h3>
            </motion.div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {[
                ...
                >
                  ...
                  <p className="text-purple-200 font-semibold">{stat.value}</p>
        ...
        {/* Action Buttons */}
        <motion.div variants={ANIMATION_VARIANTS.slideUp} className="text-center mb-8 space-x-4">
                  ...
                  {questionReviews.map((review, index) => (
                    <motion.div
                      ...
                      className={`border rounded-xl p-6 transition-all duration-300 ${
                        review.is_correct ? "border-green-500/50 bg-green-500/5" : "border-red-500/50 bg-red-500/5"
                      }`}
                    >
                      {/* Question Header */}
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-lg font-semibold text-purple-200">Question {review.question_index + 1}</h4>
                        <div
                      ...
                      <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-4 mb-6">
                        <p className="text-purple-100 text-lg leading-relaxed">{review.question}</p>
                      </div>
                      ...
                      {review.explanation && (
                        <div className="border-t border-purple-500/20 pt-4">
                          ...
                          <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
                            <p className="text-blue-200 leading-relaxed">{review.explanation}</p>
                          </div>
                        </div>
                      )}
                    </motion.div>
